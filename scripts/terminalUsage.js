  ...
  if (remoteProjectPath) {
    const result = await sendCodeToServer(
      'require("fs").promises.readdir(' +
        JSON.stringify(
          folderFilePath
            .replace(config.projectPath, remoteProjectPath)
            .replace(/\\/g, "/")
        ) +
    ...
    try {
      ...
      throw new Error(
        `Could not interpret result from server: ${JSON.stringify(result)}`
      );
    }
  }
...
async function getFileContent(filePath) {
  ...
  if (remoteProjectPath) {
    const remoteFilePath = filePath
      .replace(/\\/g, "/")
      .replace(
        config.projectPath.replace(/\\/g, "/"),
        remoteProjectPath.replace(/\\/g, "/")
      )
      .replace(/\\/g, "/");
    const result = await sendCodeToServer(
      "const targetFilePath = " +
        JSON.stringify(remoteFilePath) +
...
async function getVariableAtDate(
  ...
  const augmentedFileList = varFileList.map((entry) => ({
    ...
    isTargetInside:
      date.getTime() >= entry.fileStartDate.getTime() &&
      date.getTime() < entry.fileEndDate.getTime(),
  }));
  const firstDate = augmentedFileList[0].fileStartDate;
  const endDate = augmentedFileList[0].fileEndDate;
  const indexAfterFileList = augmentedFileList.findIndex(
    (a) => a.isTargetInside
  );
  const indexFileList =
    indexAfterFileList === -1
      ? augmentedFileList.length - 1
      : indexAfterFileList;
  const splitFileList =
    indexFileList === -1
      ? []
      : augmentedFileList.slice(indexFileList - 1, indexFileList + 2);
  ...
  const entries = targetFileList
    ? await readJsonlDataFile(targetFileList, skipCache)
    : [];
  ...
  const augmented = entries.map((entry) => ({
    ...
    isTargetInside:
      ...
      date.getTime() < entry.date.getTime() + 20_000,
  }));
  const indexAfterEntries = augmented.findIndex((a) => a.timeSinceTarget <= 0);
  const indexEntries =
    indexAfterEntries === -1 ? augmented.length - 1 : indexAfterEntries - 1;
  const splitEntries =
    indexEntries === -1
      ? []
      : augmented.slice(indexEntries - 1, indexEntries + 2);
...
/**
 * @returns {Promise<[Date | null, bigint, bigint]>}
 */
async function getUsage(date) {
  const { lastEntry, currEntry, nextEntry } = await getVariableAtDate(
    "eth-intf-sts",
    date
  );
  if (!currEntry?.isTargetInside) {
    return [null, BigInt(0), BigInt(0)];
  }
  return [
    currEntry.date,
    BigInt(currEntry.value.eth0.recv[1]),
    BigInt(currEntry.value.eth0.sent[1]),
  ];
}
...
async function init() {
  ...
  const entryRecord = {};
  function displayState(name, date, kb) {
    if (name === "sent") {
      process.stdout.write(getDateTimeString(date) + " ");
    }
    if (!entryRecord[name]) {
      entryRecord[name] = [];
    }
    entryRecord[name].push({str: getDateTimeString(date), time: date.getTime(), kb});
    if (entryRecord[name].length > 10) {
      entryRecord[name].shift();
    }
    const list = entryRecord[name];
    for (let j = 0; j < list.length; j++) {
      const i = list[j];
      process.stdout.write(`${j.toString()} ${i.str} ${kb - list[0].kb} KB in ${Math.floor((date.getTime() - list[0].time) / 1000)}`)
  ...
  const [startDate, startRecv, startSent] = await getUsage(new Date(time));
  if (!startDate || startRecv === BigInt(0) || startSent === BigInt(0)) {
    throw new Error("Failed to start");
  }
  let recvKb = BigInt(0);
  let sentKb = BigInt(0);
  let lastRecv = startRecv;
  let lastSent = startSent;
  let lastTime = startDate.getTime();

  while (true) {
    const currentTime = new Date().getTime();
    if (time + 10_000 > currentTime) {
      await new Promise((resolve) => setTimeout(resolve, 10_000));
    } else {
      await new Promise((resolve) => setTimeout(resolve, 200));
    }
    time += 10_000;
    const [date, recv, sent] = await getUsage(new Date(time));
    if (!date) {
    ...
    if (lastTime === date.getTime()) {
      console.log(
        ...
        "because of matching date"
      );
      await new Promise((resolve) => setTimeout(resolve, 1_000));
      continue;
    }
    lastTime = date.getTime();
    const deltaSentKb = (sent - lastSent) / BigInt(1024);
    const deltaRecvKb = (recv - lastRecv) / BigInt(1024);
    if (deltaSentKb >= BigInt(0) && deltaRecvKb >= BigInt(0)) {
      sentKb += deltaRecvKb;
      recvKb += deltaRecvKb;
      displayState("sent", date, sentKb);
      displayState("recv", date, recvKb);
    } else {
      console.log(
        "Ignoring",
        getDateTimeString(date),
        "because of negative delta"
      );
    }
  }
  rl.close();
